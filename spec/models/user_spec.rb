...
RSpec.describe User, type: :model do
  ...
  describe 'need_to_write_log?' do
    ...
    context '当日の記録の午後' do
      ...
      it '２回記録している場合はtrueが帰ること' do
        ...
        self_care = build(:self_care, user: user, log_date: search_date, self_care_classification: classification)
  ...
  describe 'self_cares_of_this_month' do
    ...
    before do
      log_dates.each do |d|
        self_care = build(:self_care, user: user, log_date: d, self_care_classification: classification)
        self_care.save(validate: false)
      end
    end
